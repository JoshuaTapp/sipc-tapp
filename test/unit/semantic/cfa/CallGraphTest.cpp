...
TEST_CASE("CallGraph: simple call graph with cycle(recursion)"
  ...
  REQUIRE(callGraph.get()->getTotalVertices() == 2); // 2 subroutines
  REQUIRE(callGraph.get()->getTotalEdges() ==
          2); // 1 edge from main ->foo and foo->foo

  // std::vector<std::pair<ASTFunction*, ASTFunction*>> edges =
  // callGraph.get()->getEdges(); check if there is an edge between foo and foo
...
TEST_CASE("CallGraph: test complex call graph (overapproximations)"
  ...
  REQUIRE(true ==
          callGraph.get()->existEdge("h", "f")); // check some false properties
  REQUIRE(false ==
          callGraph.get()->existEdge("f", "f")); // check some false properties
}
...
TEST_CASE("CallGraph: test getCallee by function name"
  ...
  // check the size of callees, also can be checked whether a function is being
...
TEST_CASE("CallGraph: test getCallee by ASTFunction*"
  ...
  REQUIRE(true == (callees.find(callee) !=
                   callees.end())); // h should be in the callee set
}
...
TEST_CASE("CallGraph: test getCallers"
  ...
  REQUIRE(true ==
          (callers.find("bar") != callers.end())); // bar should be in the set
}
...
TEST_CASE("CallGraph: test getCallers by ASTFunction*"
  ...
  REQUIRE(callers.size() == 1); // as bar called foo, size should be 1
  REQUIRE(true ==
          (callers.find(caller) != callers.end())); // bar should be in the set
}
...
TEST_CASE("CallGraph: test getEdges"
  ...
  REQUIRE(true == (std::find(edges.begin(), edges.end(),
                             std::make_pair(foo2, foo1)) != edges.end()));
  REQUIRE(true == (std::find(edges.begin(), edges.end(),
                             std::make_pair(bar, foo2)) != edges.end()));
}
