...
Any ASTBuilder::visitArrayConstructorExpr(TIPParser::ArrayConstructorExprContext *ctx)
{
  /*
   ...
   * If the OF() is null, then the array is implicitly constructed (e.g. [1 of 2]).
   * If the OF() is not null, then the array is explicitly constructed (e.g. [1, 2, 3]).
   *
  ...
  visitedExpr = std::make_unique<ASTArrayConstructorExpr>(std::move(arrayExprs), implicit);
...
Any ASTBuilder::visitArraySubscriptExpr(TIPParser::ArraySubscriptExprContext *ctx)
{
  visit(ctx->expr(0));
  auto array = std::move(visitedExpr);
  visit(ctx->expr(1));
  auto index = std::move(visitedExpr);
  visitedExpr = std::make_unique<ASTArraySubscriptExpr>(std::move(array), std::move(index));
...
Any ASTBuilder::visitForStmt(TIPParser::ForStmtContext *ctx)
{
  ...
  if (rangeBased)
  {
    ...
    if (step)
    {
      ...
      visitedStmt = std::make_unique<ASTForStmt>(std::move(E1), std::move(E2), std::move(E3), std::move(E4), std::move(body));
    }
    else
    {
      visitedStmt = std::make_unique<ASTForStmt>(std::move(E1), std::move(E2), std::move(E3), nullptr, std::move(body));
    }
  }
  ...
  else
  {
    ...
    visit(ctx->expr(1));
    auto E2 = std::move(visitedExpr);
    visitedStmt = std::make_unique<ASTForStmt>(std::move(E1), std::move(E2), nullptr, nullptr, std::move(body));
  }
...
Any ASTBuilder::visitPostfixStmt(TIPParser::PostfixStmtContext *ctx)
{
  ...
  visitedStmt = std::make_unique<ASTPostfixStmt>(std::move(expr), opString(ctx->op->getType()));
...
Any ASTBuilder::visitTernaryExpr(TIPParser::TernaryExprContext *ctx)
{
  ...
  visit(ctx->expr(2));
  auto E3 = std::move(visitedExpr);
  visitedExpr = std::make_unique<ASTTernaryExpr>(std::move(E1), std::move(E2), std::move(E3));
